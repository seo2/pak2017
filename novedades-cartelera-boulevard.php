<?php include('header.php') ?>
<?php include('include-top.php') ?>
<?php include('include-slider-home.php') ?>
<?php include('include-search-home.php') ?>

   <div class="titulo_seccion">
        <div class="container">
          <div class="row">
            <div class="col-sm-12 col-md-10">
                 <h4 class="divider">cartelera boulevard</h4>
            </div>
            <div class="col-sm-12 col-md-2">
              <a href="boulevard.php" class="back">< volver</a>
            </div>
          </div>
        </div>
    </div> <!-- titulo seccion -->


    <section class="main_content">
      <div class="container">
      
      <div class="row">
        <section class="cartelera clearfix">
          
            <div class="container">
              <div class="row">
                <div class="col-sm-12">
                   <img class="img-responsive visible-md visible-lg" src="assets/img/demo_img_cartelera.png" alt="" >
                   <img class="img-responsive visible-xs visible-sm" src="assets/img/demo_img_cartelera_xs.png" alt="" >
                </div>
              </div>
              <div class="row">
                <div class="content_cartelera">
                <div class="col-sm-12">
                  <div class="main_title">
                      <h3>los mejores</h3>
                      <h4 class="divider_left">tributos</h4>
                  </div>
                </div>
                  
                  <div class="evento clearfix">
                    <div class="col-sm-6">
                        <div class="fecha clearfix">
                            <div class="num">
                                <p>25</p>
                            </div>
                            <div class="mes">
                                  <p>enero</p>
                                  <p class="dia">jueves y sabado</p>
                            </div>
                        </div>
                        <div class="tit_evento">
                            <div class="triangle_top_left"> </div>
                                <p>guns and roses</p>
                        </div>
                    </div>
                    <div class="col-sm-6">
                            <div class="datos_evento">
                                <div class="hora">
                                  <p>19:30 - 20:50 </p>
                                  <p class="lugar">BOULEVARD 1</p>
                                </div>
                                <div class="desc">
                                  <p>In enim justo, rhoncus ut, imperdiet a, venenatis vitae, justo. 
                                  Nullam dictum felis eu pede mollis pretium. Integer tincidunt. 
                                  Cras dapibus. Vivamus elementum semper nisi.</p>
                                </div>
                          </div>                      
                    </div>
                    <div class="clear"></div>
                    
                  </div><!-- evento -->
                  <div class="divisor"> </div>

                  <div class="evento clearfix">
                    <div class="col-sm-6">
                        <div class="fecha clearfix">
                            <div class="num">
                                <p>25</p>
                            </div>
                            <div class="mes">
                                  <p>enero</p>
                                  <p class="dia">jueves y sabado</p>
                            </div>
                        </div>
                        <div class="tit_evento">
                            <div class="triangle_top_left"> </div>
                                <p>guns and roses</p>
                        </div>
                    </div>
                    <div class="col-sm-6">
                            <div class="datos_evento">
                                <div class="hora">
                                  <p>19:30 - 20:50 </p>
                                  <p class="lugar">BOULEVARD 1</p>
                                </div>
                                <div class="desc">
                                  <p>In enim justo, rhoncus ut, imperdiet a, venenatis vitae, justo. 
                                  Nullam dictum felis eu pede mollis pretium. Integer tincidunt. 
                                  Cras dapibus. Vivamus elementum semper nisi.</p>
                                </div>
                          </div>                      
                    </div>
                    <div class="clear"></div>
                    
                  </div><!-- evento -->
                  <div class="divisor"> </div>

                  <div class="evento clearfix">
                    <div class="col-sm-6">
                        <div class="fecha clearfix">
                            <div class="num">
                                <p>25</p>
                            </div>
                            <div class="mes">
                                  <p>enero</p>
                                  <p class="dia">jueves y sabado</p>
                            </div>
                        </div>
                        <div class="tit_evento">
                            <div class="triangle_top_left"> </div>
                                <p>guns and roses</p>
                        </div>
                    </div>
                    <div class="col-sm-6">
                            <div class="datos_evento">
                                <div class="hora">
                                  <p>19:30 - 20:50 </p>
                                  <p class="lugar">BOULEVARD 1</p>
                                </div>
                                <div class="desc">
                                  <p>In enim justo, rhoncus ut, imperdiet a, venenatis vitae, justo. 
                                  Nullam dictum felis eu pede mollis pretium. Integer tincidunt. 
                                  Cras dapibus. Vivamus elementum semper nisi.</p>
                                </div>
                          </div>                      
                    </div>
                    <div class="clear"></div>
                    
                  </div><!-- evento -->
                  <div class="divisor"> </div>
                </div> <!-- content cartelera -->

                <div class="content_promos clearfix">
                  <div class="col-sm-12">
                    <div class="main_title">
                        <h3>los mejores</h3>
                        <h4 class="divider_left">sabores</h4>
                    </div>
                  </div>

 <section class="otras_tiendas clearfix">
 
  
         <div class="grid_tiendas">
           <div class="col-sm-3">
                 <div class="item_tienda">
                         <div  class="tienda">
                             <a class="btn_tienda" href="javascript:void(0);">
                               <span id="sparta" class="overlay_img"> </span>
                               <img class="img-responsive" src="assets/img/demo_boulevard.jpg">
                             </a>
                                 <div class="box_logo_tienda">
                                   <div class="logo_tienda">
                                       <img src="assets/img/logo_costillas.png" alt="" class="img-responsive">
                                   </div> <!-- logo_tienda -->
                                 </div> <!-- box logo tienda -->
                       </div> <!--  tienda -->
               </div> <!-- item tienda -->
           </div>
           <div class="col-sm-3">
                 <div class="item_tienda">
                         <div class="tienda">
                           <a class="btn_tienda" href="javascript:void(0);">
                               <span class="overlay_img"> </span>
                               <img class="img-responsive" src="assets/img/demo_boulevard.jpg">
                             </a>
                                 <div class="box_logo_tienda">
                                   <div class="logo_tienda">
                                       <img src="assets/img/demologotienda8.jpg" alt="" class="img-responsive">
                                   </div> <!-- logo_tienda -->
                                 </div> <!-- box logo tienda -->
                       </div> <!--  tienda -->
               </div> <!-- item tienda -->
           </div>
           <div class="col-sm-3">
               <div class="item_tienda">
                       <div class="tienda">
                         <a class="btn_tienda" href="javascript:void(0);">
                             <span class="overlay_img"> </span>
                             <img class="img-responsive" src="assets/img/demo_boulevard.jpg">
                           </a>
                               <div class="box_logo_tienda">
                                 <div class="logo_tienda">
                                     <img src="assets/img/demologotienda9.jpg" alt="" class="img-responsive">
                                 </div> <!-- logo_tienda -->
                               </div> <!-- box logo tienda -->
                     </div> <!--  tienda -->
             </div> <!-- item tienda -->
           </div>
                  <div class="col-sm-3">
               <div class="item_tienda">
                       <div class="tienda">
                         <a class="btn_tienda" href="javascript:void(0);">
                             <span class="overlay_img"> </span>
                             <img class="img-responsive" src="assets/img/demo_boulevard.jpg">
                           </a>
                               <div class="box_logo_tienda">
                                 <div class="logo_tienda">
                                     <img src="assets/img/demologotienda9.jpg" alt="" class="img-responsive">
                                 </div> <!-- logo_tienda -->
                               </div> <!-- box logo tienda -->
                     </div> <!--  tienda -->
             </div> <!-- item tienda -->
           </div>
 
           <div class="clearfix"></div>
             <div class="box_ver_mas_tiendas text-center ">
                             
                            <a href="" class="btn btn-default btn_ver_mas hvr-float">
                               <h3 class="ver_mas">ver más</h3>
                               <img src="assets/img/arrow_down.png" alt="" class="img-responsive center-block arrow_down">
                           </a>
                   </div>
         </div>

           
 </section> <!-- end otras tiendas -->
 

 

       

             
                </div><!-- content promos -->

              </div><!-- row -->
            </div><!-- container -->
      </section><!-- cartelera -->
    </div><!-- row -->


<?php include('footer.php') ?>

